...
const recaptcha = require("../utilitis/captcha");
...
router.get("/login", (req, res) =>
  res.render("loginL", {
    user: null,
    title: "Login",
    pageL: pageFieldsByLang("en", "login"),
  })
);
...
router.get("/login/:lang", (req, res, next) => {
  ...
  if (lang === "en" || lang === "fr" || lang === "ar" || !lang)
    res.render("loginL", {
      user: null,
      title: "Login",
      pageL: pageFieldsByLang(lang, "login"),
    });
  else next();
});

router.get("/register", (req, res) =>
  res.render("register", { user: null, title: "Register", errors: [] })
);

router.post("/register", (req, res) => {
  const { name, lastname, email, password, password2, gender, birthDate, cin } =
    req.body;
  let errors = [];
  ...
  if (errors.length > 0) {
    ...
    createUser(userArr, true)
      .then((creation) => {
        ...
        req.flash(
          "success_msg",
          "Complete registration by verifying your email"
        );
...
router.get("/requestresetpassword/:token", function (req, res) {
  console.log(req.params);
  Token.findOne({ token: req.params.token, T_type: "reset" }).then((token) => {
    console.log(token);
    if (token) {
...
router.post("/resetpassword", async (req, res) => {
  let errors = [];
  await Token.findOne({ token: req.body.token }).then((token) => {
  ...
  await Token.findOne({ token: req.body.token }).then((token) => {
    User.findOne({ _id: token._userId }).then((user) => {
      if (user) {
        User.updateOne(
          { _id: token._userId },
          { password: hashPass(req.body.password1) }
        ).then(() => {
          Token.deleteOne({ _userId: user._id }).catch((err) =>
            console.log(err)
          );
          req.flash(
            "success_msg",
...
router.post("/registeradoctor", (req, res) => {
  let errors = [];
  const { _userId, name, lastname, password, password2, gender, birthDate } =
    req.body;
  if (
    !_userId ||
    !name ||
    ...
    if (user) {
      User.updateOne(
        ...
        .then((completedUser) => {
          ...
          res.redirect("/users/login");
          Token.deleteOne({ _userId: _userId }).catch((err) =>
            console.log(err)
          );
        })
...
router.post("/demander_rendez-vous", ensureAuthenticated, (req, res) => {
  ...
  if (_userId == req.user._id && req.user.AccountType === "patient") {
    //later ensure that he's not vaccinated yet or he's asking for 2 rendez-vous
